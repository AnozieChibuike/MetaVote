...
const WhitelistPage = () => {
  ...
  const { account, loading, setLoading, alert, setAlert, redAlert, setRedAlert } = useContext(AppContext);
  ...
  const whitelistUser = async () => {
    ...
    try {
      ...
      if (result.success)
        setAlert(<span>Voter with reg number: {registrationNumber} successfully whitelisted, click this <a target="_blank" href={`https://sepolia-blockscout.lisk.com/tx/${result.transactionHash}`} class="font-semibold underline hover:no-underline">link</a> to view the transaction on the blockchain explorer</span>)
      else 
      setRedAlert(result.error)
    } catch (error) {
      console.log(error);
  ...
  return (
    <>
      ...
      <div className="px-3">
        ...
        />
        <span className="font-thin italic text-gray-400 font-sans">A string of characters will be generated for the user give them to secure it as it will be used as an identification for voting</span>
